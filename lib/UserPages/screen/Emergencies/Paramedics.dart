...
class ParamedicsPage extends StatefulWidget {
  ...
  const ParamedicsPage({Key? key, required this.userId, required this.userRole})
      : super(key: key);
}
...
class _ParamedicsPageState extends State<ParamedicsPage>
  ...
  void initState() {
    ...
    _fetchLatestEmergency();
  }

  Future<void> _fetchLatestEmergency() async {
    final response = await http.get(Uri.parse('http://localhost:9999/emergency/last'));
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      setState(() {
        _location = data['city'];
        _chosenLocation = LatLng(
          data['location']['coordinates'][1],
          data['location']['coordinates'][0],
        );
      });
    } else {
      throw Exception('Failed to load latest emergency');
    }
  }
      ...
      body: SingleChildScrollView(
        child: Column(
          children: [
            ...
            SizedBox(
                height: 70), // Adjust the spacing to ensure content aligns correctly
            AnimatedBuilder(
              animation: _controller,
              builder: (context, child) {
                return Row(
